...
export const paymentService = {
  ...
  // Check payment status for multiple slots constrained to a specific payment_month and allowed statuses
  async checkMultipleSlotsPaymentStatusForMonth(slots: { groupId: number; memberId: number; monthDate: string }[], month: string): Promise<Set<string>> {
    try {
      if (slots.length === 0) return new Set()
      ...
      // Get the slot IDs that have payments for the selected month and allowed statuses
      const slotIds = paymentSlots.map((slot: any) => slot.id)

      const { data: payments, error: paymentError } = await supabase
        .from('payments')
        .select('slot_id, status, payment_month')
        .in('slot_id', slotIds)
        .eq('payment_month', month)
        .in('status', ['received', 'pending', 'settled'])

      if (paymentError) {
        throw new Error(`Failed to check payments: ${paymentError.message}`)
      }

      // Create a set of slots that have payments for this month
      const slotsWithPayments = new Set<string>()
      const paidSlotIds = new Set(payments?.map((p: any) => p.slot_id) || [])
      ...
      console.error('Error checking multiple slots payment status for month:', error)
      throw error
    }
  },
  ...
  async checkDuplicatePayment(paymentData: PaymentFormData): Promise<boolean> {
    try {
      const paymentMonth = paymentData.paymentMonth || new Date().toISOString().substring(0, 7)
